...
DELIMITER $$
USE `info_team08_schema`$$
CREATE PROCEDURE `nouveau_interprete` (pseudo_ VARCHAR(45), prenom_ VARCHAR(45), nom_ VARCHAR(45), dateNaissance_ VARCHAR(45))
BEGIN
...
DELIMITER $$
USE `info_team08_schema`$$
CREATE PROCEDURE `association_titre_interprete` (titre_ VARCHAR(45), pseudo_ VARCHAR(45))
BEGIN
	DECLARE idTitre_ INT;
    DECLARE pseudo_verifie_ VARCHAR(45);
    SELECT idCatalogue INTO idTitre_ FROM TitreMusical WHERE titre LIKE concat('%',titre_,'%') LIMIT 1;
    SELECT pseudonyme INTO pseudo_verifie_ FROM Interprete WHERE pseudonyme LIKE concat('%',pseudo_,'%') LIMIT 1;
	INSERT INTO `info_team08_schema`.`Discographie` (`TitreMusical_Catalogue_idCatalogue`, `Interprete_pseudonyme`) VALUES (idTitre_, pseudo_verifie_);
END$$
